...
class Bot:
    def __init__(
        ...
        logging_formeter = logging.Formatter(
        ...
        # log in
        try:
            self.instaling = Instaling(login=self.login, passwd=self.passwd)
        except LoginError:
            ...
            try:
                with open(self.path_to_words_json, "r", encoding="utf-8") as wordsfile:
                    try:
                        ...
                        self.logger.warning(
                            "Cannot decode json file, ignoring content!"
                        )

            except FileNotFoundError:
                self.logger.warning(
                    f"No such file '{self.path_to_words_json}', ignoring content!"
                )
    ...
    def start(self, delay=0):  # minutes to delay executing the session
        ...
        while True:
            ...
            if not self.isSpeedrun:
                ...
                self.logger.debug(
                    f"Typing (waiting) for {time_typing_imaginary} seconds"
                )
        ...
        try:
            with open(
                self.path_to_words_json, "r", encoding="utf-8"
            ) as wordsfile, self.lock:
            ...
            self.logger.warning(
                f"file {path_to_words_json} deleted during the execution, creating new one"
            )

        except json.JSONDecodeError:
            self.logger.warning(
                f"file {path_to_words_json} corruption during the execution, ignoring content and overwriting"
            )

        finally:
            with open(
                self.path_to_words_json, "w", encoding="utf-8"
            ) as wordsfile, self.lock:
